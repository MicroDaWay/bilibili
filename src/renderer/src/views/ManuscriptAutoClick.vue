...
const searchHandler = () => {
  ...
  const findItem = bilibiliStore.excelData.find((item) => item['投稿标签'].includes(postTag.value))
  if (findItem) {
    filterData.value = {
      ...findItem,
      ['活动开始时间']: format(excelDateToJSDate(findItem['活动开始时间']), 'yyyy-MM-dd'),
      ['活动结束时间']: format(excelDateToJSDate(findItem['活动结束时间']), 'yyyy-MM-dd')
    }
    flag = true
  }
